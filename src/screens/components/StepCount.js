...
const StepCount = ({steps, startDate, endDate}) => {
  ...
  for (let i = 0; i < steps.length; i++) {
    if (dataMap.has(moment(steps[i].startDate).format('ddd'))) {
      var value = dataMap.get(moment(steps[i].startDate).format('ddd'));
      value += steps[i].value;
      dataMap.set(moment(steps[i].startDate).format('ddd'), value);
    } else {
      dataMap.set(moment(steps[i].startDate).format('ddd'), steps[i].value);
    }
  }
  ...
  const data = {
    ...
    datasets: [
      {
        data: steps.length > 0 ? dataToShow : [null],
        colors: [
          (opacity = 1) => `#1466A8`,
          (opacity = 1) => `#1466A8`,
          (opacity = 1) => `#1466A8`,
      ...
      <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
        ...
        <Text category="h5" style={{fontWeight: 'bold'}}>
          <Text category="h5" style={{fontWeight: 'bold', color: '#D4356F'}}>
            {steps.length > 0 ? averageData : 'No Data'}
          </Text>
          <Text>{'  '}</Text>
